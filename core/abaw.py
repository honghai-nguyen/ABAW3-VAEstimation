...
def load_csv():
    path_frame = pathlib.Path('{}/extract_features/'.format(cfg.DATA_LOADER.DATA_DIR))
    dct = {}
...
class ABAW3Dataset(data.Dataset):
    def __init__(self, split='Train', transforms=None, kfold=-1):
        self.root_dir = cfg.DATA_LOADER.DATA_DIR
        ...
        if self.split != 'Test':
            ...
            data_dict = \
                np.load(pathlib.Path(self.root_dir, '{}_test.npy'.format(self.task)).__str__(), allow_pickle=True).item()
    ...
    def __getitem__(self, index):
        ...
        if cfg.MODEL.BACKBONE == 'feat+reload':
            sample['feature'] = torch.tensor(np.stack(sample['feature'])).type(torch.FloatTensor)
            sample['image'] = torch.stack(sample['image'], dim=0)
...
class ABAW3DataModule(LightningDataModule):
    def __init__(self):
        ...
        if cfg.DATA_LOADER.IMG_SIZE != 112:
            train_augs = [transforms.Resize(cfg.DATA_LOADER.IMG_SIZE), ] + train_augs
            val_augs = [transforms.Resize(cfg.DATA_LOADER.IMG_SIZE), ] + val_augs

        self.transforms_train = transforms.Compose(aug + train_augs)
        self.transforms_test = transforms.Compose(val_augs)

    def setup(self, stage=None, kfold=-1):
        if stage == 'fit' or stage is None:
            # Create train set and val set
            self.train_dataset = ABAW3Dataset(split='Train', transforms=self.transforms_train, kfold=kfold)
            self.val_dataset = ABAW3Dataset(split='Validation', transforms=self.transforms_test, kfold=kfold)
